...
from model_conv_names import get_conv_names

""" 此变量表示计算过程中的 计算顺序"""
forward_idx = 0
model_all_conv_res = get_conv_names()
...
class ChannelPruneNet(nn.Module):
    ...
    def __init__(self,model_name,channel_select_algo=None,is_teacher=True,phase=1,weight_path=None,conv_names=None,**kwargs):
        ...
        self.obtain_all_conv_names_resnet50()
        ...
        if not is_teacher:
            ...
            if model_name == 'demo':
                ...
                self.demo_input = Variable(torch.randn(1 ,3, 32, 32))
            else:
                raise ValueError
        ...
        if self.weight_path is None or self.weight_path is '':
            assert self.phase == 3, "phase must be 3 if weight_path is null"
            return
        ''' 加载权重 以及 网络结构调整、剪枝 '''
    ...
    def _make_layer(self, planes, num_blocks, stride, idx):
        strides = [stride] + [1]*(num_blocks-1)

        setattr(self, 'layer' + str(idx) + '_block_num', len(strides))

        for block_idx, stride in enumerate(strides):

            setattr(self,'layer'+str(idx)+'_block'+str(block_idx)+'_in_planes',self.in_planes)
            setattr(self,'layer'+str(idx)+'_block'+str(block_idx)+'planes',planes)
            setattr(self,'layer'+str(idx)+'_block'+str(block_idx)+'stride',stride)

            cur_in_planes = eval('self.layer'+str(idx)+'_block'+str(block_idx)+'_in_planes')
            cur_planes = eval('self.layer'+str(idx)+'_block'+str(block_idx)+'planes')
            cur_stride = eval('self.layer'+str(idx)+'_block'+str(block_idx)+'stride')

            expansion = self.expansion

            setattr(self,'layer'+str(idx)+'_block'+str(block_idx)+'_conv1',
                    nn.Conv2d(cur_in_planes, cur_planes, kernel_size=1, bias=False))
            setattr(self, 'layer' + str(idx) + '_block' + str(block_idx) + '_bn1', nn.BatchNorm2d(cur_planes))

            setattr(self, 'layer' + str(idx) + '_block' + str(block_idx) + '_conv2',
                    nn.Conv2d(cur_planes, cur_planes, kernel_size=3, stride=cur_stride, padding=1, bias=False))
            setattr(self, 'layer' + str(idx) + '_block' + str(block_idx) + '_bn2', nn.BatchNorm2d(cur_planes))

            setattr(self, 'layer' + str(idx) + '_block' + str(block_idx) + '_conv3',
                    nn.Conv2d(cur_planes, expansion * cur_planes, kernel_size=1, bias=False))
            setattr(self, 'layer' + str(idx) + '_block' + str(block_idx) + '_bn3', nn.BatchNorm2d(expansion * cur_planes))


            if cur_stride != 1 or cur_in_planes != expansion * cur_planes:
                setattr(self, 'layer' + str(idx) + '_block' + str(block_idx) + '_shortcut_conv',
                        nn.Conv2d(cur_in_planes, expansion * cur_planes, kernel_size=1, stride=cur_stride,
                                  bias=False))
                setattr(self, 'layer' + str(idx) + '_block' + str(block_idx) + '_shortcut_bn', nn.BatchNorm2d(expansion * cur_planes))



            self.in_planes = planes * expansion

    def resnet50_init(self):
        num_blocks = [3,4,6,3]
        self.in_planes = 64
        self.expansion = 4
        ...
        self._make_layer(64, num_blocks[0], stride=1, idx=1)
        self._make_layer(128, num_blocks[1], stride=2, idx=2)
        self._make_layer(256, num_blocks[2], stride=2, idx=3)
        self._make_layer(512, num_blocks[3], stride=2, idx=4)
        self.linear = nn.Linear(512 * self.expansion, 10)
    ...
    def resnet50_forward(self,x):
        global forward_idx
        global model_all_conv_res
        forward_idx = 0

        out = self.conv1(x)
        cur_conv_name = list(model_all_conv_res.keys())[forward_idx]
        out = self.exec_plugin(cur_conv_name, out)
        model_all_conv_res[cur_conv_name] = out
        forward_idx += 1
        out_pre = F.relu(self.bn1(out))

        for i in range(1,5):
            block_num = eval('self.layer' + str(i) + '_block_num')
            for block_idx in range(block_num):
                cur_conv1 = eval('self.layer'+str(i)+'_block'+str(block_idx)+'_conv1')
                cur_bn1 = eval('self.layer'+str(i)+'_block'+str(block_idx)+'_bn1')
                cur_conv2 = eval('self.layer' + str(i) + '_block' + str(block_idx) + '_conv2')
                cur_bn2 = eval('self.layer' + str(i) + '_block' + str(block_idx) + '_bn2')
                cur_conv3 = eval('self.layer' + str(i) + '_block' + str(block_idx) + '_conv3')
                cur_bn3 = eval('self.layer' + str(i) + '_block' + str(block_idx) + '_bn3')
                cur_in_planes = eval('self.layer' + str(i) + '_block' + str(block_idx) + '_in_planes')
                cur_planes = eval('self.layer' + str(i) + '_block' + str(block_idx) + 'planes')
                cur_stride = eval('self.layer' + str(i) + '_block' + str(block_idx) + 'stride')

                out = cur_conv1(out_pre)
                cur_conv_name = list(model_all_conv_res.keys())[forward_idx]
                out = self.exec_plugin(cur_conv_name, out)
                model_all_conv_res[cur_conv_name] = out
                forward_idx += 1
                out = F.relu(cur_bn1(out))
                ...
                if cur_stride != 1 or cur_in_planes != self.expansion * cur_planes:
                    cur_shortcut_conv = eval('self.layer' + str(i) + '_block' + str(block_idx) + '_shortcut_conv')
                    cur_shortcut_bn = eval('self.layer' + str(i) + '_block' + str(block_idx) + '_shortcut_bn')

                    out_shortcut = cur_shortcut_conv(out_pre)
                    cur_conv_name = list(model_all_conv_res.keys())[forward_idx]
                    out_shortcut = self.exec_plugin(cur_conv_name, out_shortcut)
                    model_all_conv_res[cur_conv_name] = out_shortcut
                    forward_idx += 1
                    out += cur_shortcut_bn(out_shortcut)

                out_pre = F.relu(out)

        out = F.avg_pool2d(out_pre, 4)
        out = out.view(out.size(0), -1)
        out = self.linear(out)
        return F.log_softmax(out),model_all_conv_res
    ...
    def obtain_all_conv_names_resnet50(self):
        added_names = []
